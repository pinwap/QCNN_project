    ...
    def rotate(self, current_bit, best_bit, fitness_current, fitness_best):
        ...
        if current_bit == 0 and best_bit == 0:
            ...
            if not is_current_F_better:
                ...
                d_theta = delta
                # sigh
                if abs(self.beta) < eps: sign = 0
                elif abs(self.alpha) < eps: sign = np.random.choice([-1, 1]) #sign +-1  
                elif self.alpha * self.beta > 0: sign = -1
                elif self.alpha * self.beta < 0: sign = 1
            ...
            if not is_current_F_better:
                ...
                d_theta = delta
                # sign
                if abs(self.alpha) < eps: sign = 0  
                elif abs(self.beta) < eps: sign = np.random.choice([-1, 1]) #sign +-1
                elif self.alpha * self.beta > 0: sign = 1
                elif self.alpha * self.beta < 0: sign = -1
...
class QuantumChromosome:
    ...
    def collapse(self):
        ...
        self.structure_code = [(self.binary_code[i] << 1) | self.binary_code[i+1] for i in range(0, len(self.binary_code), 2)]
